...
function Talented:LearnTalent(template, tab, index)
    ...
    if not p.always_call_learn_talents then
        ...
        if state == "full" or -- talent maxed out
        state == "unavailable" or -- prerequisites not fullfilled
        -- UnitCharacterPoints("player") == 0 -- no more points
        GetUnspentTalentPoints(nil, is_pet, is_pet and GetActiveTalentGroup(nil, is_pet) or
            (C_SpecializationInfo.GetActiveSpecGroup and C_SpecializationInfo.GetActiveSpecGroup() or
                GetActiveTalentGroup())) == 0 -- no more points
        then
            return
        end
    end
    ...
    ShowDialog(L["Are you sure that you want to learn \"%s (%d/%d)\" ?"]:format(
        self:GetTalentName(template.class, tab, index), template[tab][index] + 1,
        self:GetTalentRanks(template.class, tab, index)), tab, index, is_pet)
end
