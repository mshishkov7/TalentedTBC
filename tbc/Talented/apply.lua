...
function Talented:ApplyCurrentTemplate()
    ...
    elseif count > available then
        self:Print(
            L["Sorry, I can't apply this template because you don't have enough talent points available (need %d)!"],
            count)
        self.mode = "view"
        self:UpdateView()
    else
...
function Talented:ApplyNextTalentPoint()
    ...
    local group
    if pet then
        group = GetActiveTalentGroup(nil, pet)
    else
        group = C_SpecializationInfo.GetActiveSpecGroup and C_SpecializationInfo.GetActiveSpecGroup() or
...
function Talented:CheckTalentPointsApplied()
    ...
    local current
    if pet then
        current = self.pet_current
    else
        current = self:GetActiveSpec()
    end
    local group = GetActiveTalentGroup(nil, pet)
    local failed
    for tab, tree in ipairs(self:UncompressSpellData(template.class)) do
