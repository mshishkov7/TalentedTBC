local Talented = LibStub("AceAddon-3.0"):NewAddon("Talented", "AceConsole-3.0", "AceComm-3.0", "AceHook-3.0",
...
function Talented:MakeTarget(targetName)
    ...
    if not self:ValidateTemplate(target) or
        ...
        (not RAID_CLASS_COLORS[target.class] and (not self.GetPetClass or target.class ~= self:GetPetClass())) then
        self.db.char.targets[targetName] = nil
        return nil
    end
    target.name = name
...
function Talented:OnEnable()
    ...
    -- Hook the function that creates the base frame to set the OnClick handler for the activate button
    self:SecureHook("CreateBaseFrame")
end

function Talented:CreateBaseFrame_PostHook(frame)
    if frame and frame.bactivate then
        frame.bactivate:SetScript("OnClick", function(self)
            local talentGroup = self.talentGroup -- This property is set in TalentView:Update()
            if talentGroup then
                if C_SpecializationInfo and C_SpecializationInfo.SetActiveSpecGroup then
                    ...
                    SetActiveTalentGroup(talentGroup) -- Fallback for older APIs
                end
            end
        end)
    end
