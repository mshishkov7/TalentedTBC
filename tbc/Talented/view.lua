...
function TalentView:Update()
    ...
    local maxpoints
    if self.pet then
        maxpoints = GetMaxPoints(nil, true, self.spec) -- Pet talents still use GetMaxPoints with pet argument
    else
        maxpoints = GetMaxPoints(nil, nil, C_SpecializationInfo.GetActiveSpecGroup and
            C_SpecializationInfo.GetActiveSpecGroup() or self.spec)
    end
    ...
    if checkbox then
        if template.talentGroup == GetActiveTalentGroup() then
            if activateBtn then
                activateBtn:Hide()
                -- Ensure the OnClick handler is cleared when hidden to prevent stale closures
                activateBtn:SetScript("OnClick", nil)
            end
            roleSelectBtn:Show()
            checkbox:Show()
        ...
        elseif template.talentGroup then
            ...
            roleSelectBtn:Hide()
            if activateBtn then
                -- The OnClick handler is set once in CreateBaseFrame_PostHook
...
function TalentView:UpdateTalent(tab, index, offset)
    ...
    if self.mode ~= "edit" and self.spec then
        return
    end
